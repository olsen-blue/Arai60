...
# Step5-1：バイナリサーチを理解し直す
- 閉区間 [0, len(long_nums)] バージョン
```python
class Solution:
    def intersection(self, nums1: List[int], nums2: List[int]) -> List[int]:
        if not nums1 or not nums2:
        ...
        for num in short_nums:
            ...
            while left <= right:    #「left = right」つまり区間に要素が１つの時も、whileの中の処理をしたい、だから「<=」とする。
                middle = (left + right) //2
                if long_nums[middle] == num:
                    intersections.append(num)
                    break
...
class Solution:
    def intersection(self, nums1: List[int], nums2: List[int]) -> List[int]:
        ...
        for num in short_nums:
            ...
            left = 0
            right = len(long_nums)
            while left + 1 <= right:            #「left + 1 = right」つまり区間に要素が１つの時も、whileの中の処理をしたい、だから「<=」とする。
                middle = (left + right) //2
                if long_nums[middle] == num:
                    intersections.append(num)
                    break
