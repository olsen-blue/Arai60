...
## 解法1:トップダウン再帰(関数なしver.):AC
 - https://github.com/hayashi-ay/leetcode/pull/46/files
   ...
   - でも、int(not self.~~~)って見づらい? -> 最終的には、Step3で、1 - self.kthGrammar()の方が良いと感じました。
...
```
 ...
 - 二分木構造上でもイメージできて、すごく直感的な解法だった。これは非常に好きですね。二分木というか A, B の2ブロックのイメージで書ける。
...
## 解法2':反転2ブロック(A, B)の利用(n遷移あり):AC
 - こっちの方が解法1より、好きかも。
 - 解法1のようなミクロな視点より、解法2'のようなマクロの視点でA, Bブロックが見えている方が、なんか見晴らしが良い感じがする。
```
 n-1 |   A
  n  | A + B(<-Aの反転)
```
